...
async function tryGetDiagnostics(document, svelteDoc, settings) {
    ...
    try {
        ...
        return (res.stats.warnings || res.warnings || [])
            ...
            .map((diag) => (0, documents_1.mapObjWithRangeToOriginal)(transpiled, diag))
...
async function createParserErrorDiagnostic(error, document) {
    ...
    if (diagnostic.message.includes('expected')) {
        const isInStyle = (0, documents_1.isInTag)(diagnostic.range.start, document.styleInfo);
        const isInScript = (0, documents_1.isInTag)(diagnostic.range.start, document.scriptInfo || document.moduleScriptInfo);
...
function adjustMappings(diag, doc) {
    ...
    diag.range = (0, utils_1.moveRangeStartToEndIfNecessary)(diag.range);
    if (diag.code === 'css-unused-selector' &&
        doc.styleInfo &&
        !(0, documents_1.isInTag)(diag.range.start, doc.styleInfo)) {
